...
export function AppHeader({campName,view,query,setQuery,results,selectCamper,importFile,canImport}:Props){return <header><div><small>{campName||"CAMPKIN"}</small><h1>{view}</h1></div><div className="search"><Search size={16}/><input aria-label="Search campers" placeholder="Search campers" value={query} onChange={e=>setQuery(e.target.value)}/>{results.length>0&&<div className="results">{results.map(camper=><button key={camper.id} onClick={()=>selectCamper(camper)}><Avatar c={camper}/><span><b>{camper.name}</b><small>{camper.room||"Unassigned"} | {camper.group||"No group"}</small></span></button>)}</div>}</div><button className="primary" onClick={importFile} disabled={!canImport}><Upload size={16}/>Import file</button></header>}
